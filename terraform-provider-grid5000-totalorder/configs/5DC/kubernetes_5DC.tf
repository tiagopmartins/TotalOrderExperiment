 locals{
    depends_on = [grid5000_deployment.my_deployment1,grid5000_deployment.my_deployment2,grid5000_deployment.my_deployment3,grid5000_deployment.my_deployment4,grid5000_deployment.my_deployment5]

    nodes_list    = setunion(grid5000_deployment.my_deployment1.nodes, grid5000_deployment.my_deployment2.nodes, grid5000_deployment.my_deployment3.nodes, grid5000_deployment.my_deployment4.nodes, grid5000_deployment.my_deployment5.nodes)
    deployment_list = [grid5000_deployment.my_deployment1, grid5000_deployment.my_deployment2, grid5000_deployment.my_deployment3, grid5000_deployment.my_deployment4, grid5000_deployment.my_deployment5]
    ...
    deployment5 = [ for node in local.deployment_list[4].nodes : {
                    address = node
                    internal_address = node
                    role = ["worker"]
                    labels = {
                      dc = local.deployment_list[4].site
                    }
              }]
    joint_deployment = concat(local.deployment1, local.deployment2, local.deployment3, local.deployment4, local.deployment5)
}
...
output "deployment5_print" {
  value = local.deployment5
}
