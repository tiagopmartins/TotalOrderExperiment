...
def scale_replica_set(client, selector, scale):
    replica_set_list = client.list_namespaced_replica_set(namespace=NAMESPACE,
                                          label_selector=selector).items
    s = client.read_namespaced_replica_set_scale(replica_set_list[0].metadata.name, NAMESPACE)
    s.spec.replicas = scale
    client.patch_namespaced_replica_set_scale(replica_set_list[0].metadata.name, NAMESPACE, s)
...
def copy_file_to_pod(client, file_path, pod_name, pod_path, container):
    tar_command = 'tar xmvf - -C ' + pod_path
    exec_command = ['/bin/bash', '-c', tar_command]
