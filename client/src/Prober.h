...
private:
    ...
    std::vector<std::vector<uint64_t>>* _times;   // Mapping between addresses and times (per second)
...
public:
     ...
     */
    uint64_t sendProbingMessage(std::string address);
     ...
     */
    std::vector<std::vector<uint64_t>>* stability(std::string address, int duration);
